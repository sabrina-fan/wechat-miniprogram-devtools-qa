...
description: Protocol-first acceptance and debugging for WeChat Mini Programs in WeChat Developer Tools. Use when you need to build or open a mini-program in DevTools, inspect the simulator, verify real page behavior, automate routes and DOM geometry, validate uploads or API-backed flows, or diagnose DevTools/runtime rendering problems. Prefer the DevTools CLI, miniprogram-automator, local APIs, and read-only database probes; use Computer Use only as the final fallback when every reliable protocol path is unavailable. Do not use for writing or modifying mini-program source code, deploying to production, or routine development without an acceptance or rendering question to answer.
---
...
1. Resolve the repository root, the mini-program source, the release build directory, and the local API before acting. In a typical monorepo these are usually `apps/miniprogram`, `apps/miniprogram/dist/build/mp-weixin`, and `http://127.0.0.1:3000/api/v1`; detect them from `package.json` and the project structure instead of assuming fixed paths.
...
Native layers are independent rendering surfaces. An always-present empty `page-container` can cover a Vue page even when `overlay=false` and the Vue nodes have valid geometry; preserve the leave guard but make the surface visually inert (transparent, minimal dimensions, and zero opacity) and verify that `beforeleave` still intercepts the intended back gesture.

Treat `video`, map, canvas, and other native components the same way: stop or unmount them on hide/unload when they can outlive the page. For bundled media, use the package asset directly instead of copying one file per component instance into `USER_DATA_PATH`; retain `saveFile` only for real business files such as recordings or PDFs.
...
Read [references/acceptance-protocol.md](references/acceptance-protocol.md) when performing the detailed acceptance workflow or when a DevTools/API boundary fails.
